...
function writeEventToScreen(eventString) {
	...
	/*$(document).addEventListener('keypress', (event)=>{
		var name = event.key;
		var code = event.code;
		if (code==100 || code==119 || code==109 || code==121 || code==110)
		{
			var eventstring = String.fromCharCode(code);
		}
	})*/
	//console.log($('#new-event-text').trigger('log', ['repeatchoice', { 'key pressed': eventstring }]));
	$('#new-event-text').trigger('log', ['eventCreated', { 'what': 'Event created successfully' }])
	$('#new-event-text').text(eventString)
	$('#add-event-button').attr('disabled', false)
